...
#     # Convert 'SEM_EXPIRY_DATE' to datetime
#     df['SEM_EXPIRY_DATE'] = pd.to_datetime(df['SEM_EXPIRY_DATE'])

#     # Sort by symbol and expiry date
#     df = df.sort_values(by=['SEM_SYMBOL', 'SEM_EXPIRY_DATE'])
    
#     data_dict = {}
    
#     # Group by 'SEM_SYMBOL' and get the row with the earliest expiry date
#     for symbol, group in df.groupby('SEM_SYMBOL'):
#         earliest_row = group.iloc[0]  # Get the first row (earliest expiry date)
#         trading_symbol = earliest_row['SEM_SYMBOL']
#         exm_exch_id = earliest_row['SEM_SCRIP_CODE']  # Replace with correct column name if needed
